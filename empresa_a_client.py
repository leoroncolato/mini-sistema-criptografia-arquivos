...
logger = CryptoLogger.setup_logger("Empresa_A")
...
def setup_keys():
    ...
    try:
        ...
        logger.info("[Empresa A] Chaves carregadas do disco.")
    except FileNotFoundError:
        logger.info("[Empresa A] Gerando novas chaves RSA...")
...
def main():
    logger.info("=== EMPRESA A (CLIENTE) INICIADA ===")
    ...
    try:
        ...
        logger.info("Inicie o servidor da Empresa B primeiro para que ela gere sua chave pública!")
        return
    ...
    logger.info(f"[Empresa A] Lendo arquivo: {caminho_pdf}")
    ...
    logger.info(f"[Empresa A] Conectando ao servidor em {host}:{port}...")
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            s.connect((host, port))
            s.sendall(dados_para_envio)
            logger.info("[Empresa A] Pacote criptografado enviado com sucesso pela rede!")
        except ConnectionRefusedError:
            logger.info("[ERRO] Não foi possível conectar. Certifique-se de que a Empresa B está rodando.")
